...
# Run only the exploratory susieR finite-reference + EB mismatch extension.
...
J_grid <- as.integer(numeric_grid("COAL_BENCH_J", c(500, 1000, 2000)))
...
run_task <- function(index) {
  ...
  status <- system2(
    ...
    c(
      "--vanilla",
      file.path("experiments", "coalescent-susie-rss-eb-benchmark-worker.R"),
  ...
  data.table(
    ...
    panel_seed = task$panel_seed, status = status, log_file = log_file
  )
}

results <- if (.Platform$OS.type == "unix" && cores > 1L) {
  rbindlist(parallel::mclapply(
    seq_len(nrow(tasks)), run_task, mc.cores = cores, mc.preschedule = FALSE
  ))
} else {
