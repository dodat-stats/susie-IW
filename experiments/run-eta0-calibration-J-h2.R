...
J_grid <- as.integer(numeric_grid(
  "ETA0_CAL_J_GRID", c(100, 250, 500, 1000, 2000)
))
panel_seeds <- as.integer(numeric_grid(
  "ETA0_CAL_PANEL_SEEDS", c(31, 32)
))
cores <- as.integer(Sys.getenv("ETA0_CAL_CORES", unset = "2"))
output_directory <- Sys.getenv(
  "ETA0_CAL_OUTPUT_DIRECTORY",
  unset = file.path("experiments", "eta0-calibration-J-h2-workers")
)
...
run_task <- function(index) {
  ...
  status <- system2(
    ...
    c(
      "--vanilla",
      file.path("experiments", "eta0-calibration-J-h2-worker.R"),
